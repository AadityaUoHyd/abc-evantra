...
const OrganizerManageEventPage: React.FC = () => {
      ...
      const fetchEvent = async () => {
        ...
        setEventData({
          ...
          ticketTypes: event.ticketTypes.map((ticket, index) => ({
            key: isTempId(ticket.id) ? `temp-${index}` : ticket.id,
            id: ticket.id,
            name: ticket.name,
  ...
  const handleEventUpdateSubmit = async (accessToken: string, id: string) => {
    const ticketTypes: UpdateTicketTypeRequest[] = eventData.ticketTypes.map(
      (ticketType, index) => ({
        key: isTempId(ticketType.id) ? `temp-${index}` : ticketType.id,
        id: isTempId(ticketType.id) ? undefined : ticketType.id,
        name: ticketType.name,
        price: ticketType.price,
        gstRate: ticketType.gstRate,
        discountRate: ticketType.discountRate,
    ...
    try {
      await updateEvent(accessToken, id, request, image || undefined);
  ...
  const handleEventCreateSubmit = async (accessToken: string) => {
    ...
    try {
      await createEvent(accessToken, request, image || undefined);
      navigate("/dashboard/events");
    } catch (err) {
      if (err instanceof Error) {
        setError(err.message);
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="container mx-auto px-4 py-8 max-w-xl">
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          ...
          <div>
            <div>
              <label htmlFor="event-name" className="text-sm font-medium text-purple-500">
                Event Name
              </label>
              <Input
                id="event-name"
          ...
          {/* Event Description */}
          <div>
            <label htmlFor="event-description" className="text-sm font-medium text-purple-500">
              Event Description
            </label>
            <Textarea
            ...
            />
            <p className="text-gray-400 text-xs">
              Details about the event description, please include as much detail as possible.
            </p>
          </div>

          {/* Event Image */}
          <div className="text-white">
            <label htmlFor="event-image" className="text-sm font-medium text-purple-500">
              Event Image{" "}
            </label>
            ...
            <label
              htmlFor="event-image"
              className="mt-2 inline-block cursor-pointer bg-yellow-500 text-black px-4 py-2 rounded hover:bg-yellow-600 transition"
            >
              Choose File
            </label>
            {image && (
              <p className="text-sm text-green-400 mt-1">{image.name}</p>
            )}
          ...
          <div>
            <label className="text-sm font-medium text-purple-500">Event Start</label>
            <DateTimeSelect
          ...
          {/* Event End Date Time */}
          <div>
            <label className="text-sm font-medium text-purple-500">Event End</label>
            <DateTimeSelect
          ...
          <div>
            <label htmlFor="venue-details" className="text-sm font-medium text-purple-500">
              Venue Details
            </label>
            <Textarea
              id="venue-details"
          ...
          <div>
            <label className="text-sm font-medium text-purple-500">Event Sales Start</label>
            <DateTimeSelect
          ...
          <div>
            <label className="text-sm font-medium text-purple-500">Event Sales End</label>
            <DateTimeSelect
            ...
            />
            <p className="text-gray-400 text-xs">
              The date and time that ticket sales end for the event.
            </p>
          </div>
              ...
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <CardHeader>
                  <div className="flex justify-between">
                    <CardTitle className="flex gap-2 items-center text-sm text-purple-500">
                      <Ticket />
                      Ticket Types
                    </CardTitle>
                    <Button
                ...
                <DialogContent className="bg-gray-900 border-gray-700 text-white">
                  <DialogHeader>
                    <DialogTitle className="text-purple-500">Add Ticket Type</DialogTitle>
                  ...
                  <div className="space-y-1">
                    <Label htmlFor="ticket-type-name" className="text-purple-500">Name</Label>
                    <Input
                      id="ticket-type-name"
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="space-y-1 w-full">
                      <Label htmlFor="ticket-type-price" className="text-purple-500">Price</Label>
                      <Input
                    ...
                    {/* gstRate */}
                    <div className="space-y-1 w-full">
                      <Label htmlFor="ticket-type-gstRate" className="text-purple-500">GST Rate</Label>
                      <Input
                        id="ticket-type-gstRate"
                        type="number"
                        value={currentTicketType?.gstRate ?? 0}
                        onChange={(e) =>
                          setCurrentTicketType(
                            currentTicketType
                              ? {
                                  ...currentTicketType,
                                  gstRate: e.target.value === "" ? 0 : Number.parseFloat(e.target.value),
                                }
                              : undefined,
                          )
                        }
                    ...
                    {/* discountRate */}
                    <div className="space-y-1 w-full">
                      <Label htmlFor="ticket-type-discountRate" className="text-purple-500">Discount Rate</Label>
                      <Input
                        id="ticket-type-discountRate"
                        type="number"
                        value={currentTicketType?.discountRate ?? 0}
                        onChange={(e) =>
                          setCurrentTicketType(
                            currentTicketType
                              ? {
                                  ...currentTicketType,
                                  discountRate: e.target.value === "" ? 0 : Number.parseFloat(e.target.value),
                                }
                              : undefined,
                          )
                        }
                    ...
                    {/* Total Available */}
                    <div className="space-y-1 w-full">
                      <Label htmlFor="ticket-type-total-available" className="text-purple-500">
                        Total Available
                      </Label>
                      <Input
                        ...
                        onChange={(e) =>
                          setCurrentTicketType(
                            currentTicketType
                              ? {
                                  ...currentTicketType,
                                  totalAvailable: e.target.value === "" ? 0 : Number.parseFloat(e.target.value),
                                }
                              : undefined,
                          )
                        }
                  ...
                  <div className="space-y-1 ">
                    <Label htmlFor="ticket-type-description" className="text-purple-500">Description</Label>
                    <Textarea
          ...
          {/* Status */}
          <div className="space-y-1">
            <Label className="text-purple-500">Status</Label>
            <Select
          ...
          <div>
            <Button className="bg-purple-700 hover:bg-purple-500" type="submit">
